...
const g_formatStr =
  "课程名：%s\n上课时间：%s:%s\n地点：%s\n授课老师：%s\n其它信息：%s";
const g_timeRegexp = new RegExp(
  "^(MON|TUE|WED|THU|FRI|SAT|SUB)\\s([0-9]{1,2}):([0-9]{1,2})$",
  "i"
);
...
function procCourses(ctx, courseGroup) {
  ...
  for (let course of courseGroup.courses) {
    ...
    let time = parseTimeAndAdvance(
      course,
      courseGroup.advance ? courseGroup.advance : "10m"
    );
...
function parseTimeAndAdvance(course, advance) {
  ...
  if (!course.time || !course.name) {
    throw new ConfigSyntaxError(
      "The name and time are not provided in config."
    );
  }
  ...
  // 计算「提醒时间」
  let temp = dayjs()
    .set("hour", parseInt(time.hour))
    .set("minute", parseInt(time.minute))
    .set("day", g_weekMap[time.week]);
  parseAdvance(
    time,
    course.advance ? course.advance.toString() : advance.toString()
  );
